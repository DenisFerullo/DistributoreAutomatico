...
// -------------------------------------------------------------------------------------------------- //
	...
	public List<ProductDto> getInventory() {
		return productRepository.findAll()
					.stream()
					.map(
							ProductMapperDto::productToProductDto
					)
					.toList();
	}
	...
	public ProductDto getByNome(String nome) {
		return ProductMapperDto.productToProductDto(
								productRepository.findByNome(nome)
				);
	}
		...
		return productRepository.findById(id)
				.map(p -> {
							...
							return ProductMapperDto
									.productToProductDto(
														productRepository.save(p)	/* updatedProduct */
									);
						}
				).orElse(null);
	}
	...
	public ProductDto addToInventory(ProductDto pDto) {
		return ProductMapperDto.productToProductDto(
									productRepository.save(
													ProductMapperDto.newProduct(pDto)
									)
				);
	}
